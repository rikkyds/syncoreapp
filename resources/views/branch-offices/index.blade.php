            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between mb-4">
                        <h3 class="text-lg font-semibold">{{ __('Branch Office List') }}</h3>
                        ...
                        <table class="min-w-full table-auto">
                            <thead>
                                <tr class="bg-gray-100">
                                    <th class="px-6 py-3 text-left">{{ __('Company') }}</th>
                            ...
                            <tbody>
                                @foreach($branchOffices as $office)
                                    <tr class="border-b">
                                        <td class="px-6 py-4">{{ $office->company->name }}</td>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('branch-offices.show', $office) }}" class="text-blue-600 hover:text-blue-900">{{ __('View') }}</a>
                                                <a href="{{ route('branch-offices.edit', $office) }}" class="text-yellow-600 hover:text-yellow-900">{{ __('Edit') }}</a>
                                                <form action="{{ route('branch-offices.destroy', $office) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('{{ __('Are you sure you want to delete this branch office?') }}')">
                                                        {{ __('Delete') }}
                                                    </button>
                                                </form>
                                            </div>
