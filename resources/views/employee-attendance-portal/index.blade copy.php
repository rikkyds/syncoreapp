<x-app-layout>
    ...
    <!-- Redesigned Header with Proper Spacing -->
    <div class="bg-blue-600 -mt-6 mb-8" style="background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 50%, #60a5fa 100%);">
        <div class="px-6 py-16">
            <div class="max-w-md mx-auto">
                <div class="flex items-center justify-between">
                    <!-- Employee Info with Generous Spacing -->
                    <div class="flex items-center">
                        <div class="mr-4">
                            @if($employee->profile_photo)
                                <img src="{{ asset('storage/' . $employee->profile_photo) }}" alt="{{ $employee->full_name }}" class="h-16 w-16 rounded-xl object-cover border-2 border-white shadow-lg">
                            @else
                                <div class="h-16 w-16 rounded-xl bg-white flex items-center justify-center shadow-lg">
                                    <span class="text-2xl font-bold text-blue-600">{{ substr($employee->full_name, 0, 1) }}</span>
                                </div>
                            @endif
                        </div>
                        <div>
                            <p class="text-xs text-blue-100 font-medium">{{ $employee->position->name ?? 'Staff' }}</p>
                            @else
                                <div class="h-16 w-16 rounded-xl bg-gradient-to-br from-white to-blue-50 flex items-center justify-center shadow-lg border-2 border-white">
                                    <span class="text-2xl font-bold text-blue-600">{{ substr($employee->full_name, 0, 1) }}</span>
                                </div>
                            @endif
                        </div>
                        <div>
                            <p class="text-xs text-blue-100 font-medium opacity-90">{{ $employee->position->name ?? 'Staff' }}</p>
                            <h3 class="font-bold text-white text-xl">{{ $employee->full_name }}</h3>
                        </div>
                    </div>
        ...
        <div class="absolute bottom-0 left-0 w-full overflow-hidden" style="transform: translateY(50%);">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320" preserveAspectRatio="none" class="w-full h-16">
                <path fill="#ffffff" fill-opacity="1" d="M0,160L48,144C96,128,192,96,288,96C384,96,480,128,576,149.3C672,171,768,181,864,165.3C960,149,1056,107,1152,101.3C1248,96,1344,128,1392,144L1440,160L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
            </svg>
        </div>
    </div>
